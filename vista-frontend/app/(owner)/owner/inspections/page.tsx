...
export default function OwnerInspectionsPage() {
  return (
    <>
      <PageHeader title="Inspection calendar" description="Every visit across every listing. Conflict prevention is on by default." />
      <div className="px-6 lg:px-8 py-8">
                  ...
                  <li key={ins.id} className="flex items-center gap-4 p-5">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-fg">
                        <Link href={`/owner/listings/${l.id}`} className="hover:text-brand">{l.title}</Link>
                      </p>
        ...
        <p className="mt-6 text-xs text-fg-subtle">
          ...
          Tip: every booked inspection notifies the assigned agent (or you) in real time via Kafka.
        </p>
      </div>
    </>
  );
