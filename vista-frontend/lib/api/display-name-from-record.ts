...
export function pickExplicitDisplayFromRecord(
  ...
  for (const [k, v] of Object.entries(rec)) {
    ...
    if (
      !full ||
      s.localeCompare(full, undefined, { sensitivity: "accent" }) !== 0
    ) {
      return s;
    }
  }
...
export function pickStructuredFirstNameFromRecord(
  rec: Record<string, unknown>,
): string {
...
export function pickStructuredLastNameFromRecord(
  rec: Record<string, unknown>,
): string {
 ...
 */
export function refineDisplayAgainstStructuredNames(
  display: string,
  rec: Record<string, unknown>,
): string {
  ...
  if (
    given &&
    tokens[0]!.localeCompare(given, undefined, { sensitivity: "accent" }) === 0
  ) {
    return given;
  }
  return out;
 ...
 */
export function shortDisplayWhenMirrorsFull(
  display: string,
  full: string,
  rec?: Record<string, unknown>,
): string {
  ...
  if (disp.localeCompare(f, undefined, { sensitivity: "accent" }) !== 0)
    return disp;
...
function inferGivenNameFromFullName(rec: Record<string, unknown>): string {
  ...
  if (family) {
    ...
    if (
      first &&
      first.localeCompare(family, undefined, { sensitivity: "accent" }) === 0
    ) {
      return parts[1] ?? first;
    }
    if (
      last &&
      last.localeCompare(family, undefined, { sensitivity: "accent" }) === 0
    ) {
      return parts[0] ?? "";
    }
  }
